...
import { ProjectStore, deleteProject } from "@/store/reducer/Projects";
...
export default function tableData({
  ...
  const { listProject, totalProject } = dataProjects;
  ...
  function getUserFromPro(project: ProjectType) {
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {Array.isArray(listProject) &&
              [...listProject].map((proObj: ProjectType, index: number) => (
                <tr
                  key={index}
                ...
                >
                  ...
                  >
                    {proObj.projectUsers &&
                      proObj.projectUsers.map((proUser: any, ind) => (
                  ...
                  <td className="px-6 py-4">
                      ...
                      onClick={() => {
                        if (!isLoading) {
                          ...
                          setBodyProject((prevValue: BodyProject) => {
                            return {
                              ...prevValue,
                              defaultProject: proObj.name,
                              currentIdProject: proObj.id,
                              selectedUserId: getUserFromPro(proObj),
                            };
                          });
                        }
                      }}
                    ...
                    <button
                      onClick={() => deleteNoteMethod(proObj.id)}
